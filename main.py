...
def new_test():
    ...
    def correct_answer_1():
        ...
        Two_test.adding_new_question(ID_test, rec_queschen, rec_answerre_1, rec_answerre_2, rec_answerre_3,
                                     correct_answer, question_numbers)
    ...
    def exit_for_test():
        global contin
        global test_number
        messagebox.showinfo('ТЕСТ СОЗДАН' + '                     ',
                            'ID ТЕСТА:  ' + str(test_number) + '                         ')
        contin.destroy()
        main_window()
            ...
            if result_check_ID == True:
                ...
                if list_question_answer != 0:
                    ...
                    if list_question_answer[3] != '':
                        ...
                        for_answere_1 = Label(go_exist_test, text=list_question_answer[3], font=label_font,
                                              **base_padding)
                        for_answere_1.place(x=73, y=110, width=500)

                        if list_question_answer[4] != '':

                            for_answere_2 = Label(go_exist_test, text=list_question_answer[4], font=label_font,
                                                  **base_padding)
                            for_answere_2.place(x=73, y=150, width=500)

                            if list_question_answer[5] != '':
                                for_answere_2 = Label(go_exist_test, text=list_question_answer[5], font=label_font,
                                                      **base_padding)
        ...
        if checking_symbol_in_username == -1 and checking_symbol_in_password == -1:
            ...
            if check_name == True:
                ...
                messagebox.showwarning(title='Ошибка',
                                       message="Ваш пароль слишком короткий или такой пользователь уже существует")

        else:
            print(False)
            messagebox.showwarning(title='Ошибка',
                                   message="Вы использовали запрещенные символы (такие как: /, \, *, ~)")
